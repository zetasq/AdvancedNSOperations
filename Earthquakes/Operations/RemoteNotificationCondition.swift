...
private let RemoteNotificationQueue = APPLOperationQueue()
...
struct RemoteNotificationCondition: OperationCondition {
    ...
    func dependency(for operation: APPLOperation) -> Foundation.Operation? {
        return RemoteNotificationPermissionOperation(application: application, handler: { _ in })
    }
    
    func evaluate(for operation: APPLOperation, completion: @escaping (OperationConditionResult) -> Void) {
        /*
...
*/
private class RemoteNotificationPermissionOperation: APPLOperation {
    let application: UIApplication
